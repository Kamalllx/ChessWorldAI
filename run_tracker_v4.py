...
class SmartTrackerV4:
    ...
    def _detect_ensemble(self, frame: np.ndarray) -> List[Dict]:
        all_detections = []
        ...
        return self._cluster_detections(all_detections)
        
    def _cluster_detections(self, detections: List[Dict], dist_threshold: float = 35) -> List[Dict]:
        if len(detections) <= 1:
            return detections
    ...
    def process(self, frame: np.ndarray) -> Optional[str]:
        ...
        self.frame_count += 1
        detections = self._detect_ensemble(frame)
    ...
    def pgn(self, name: str) -> str:
        game = chess.pgn.Game()
        game.headers["Event"] = "ChessWorldAI V4"
...
def main():
    parser = argparse.ArgumentParser(description="ChessWorldAI V4 Tracker")
    ...
    output = args.output or f"output/pgn/{video.stem}_v4.pgn"
    ...
    tracker = SmartTrackerV4(model_path=model_path)
    ...
    try:
        while True:
            ...
            cv2.imshow("ChessWorldAI V4", vis)
