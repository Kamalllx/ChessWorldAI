#!/usr/bin/env python3
"""
ChessWorldAI - Smart Tracker V5
Detect-Lock-Track approach with improved detection
"""
...
class SmartTrackerV5:
    def __init__(self, model_path: str = 'models/pieces.pt'):
        ...
        self.phase = TrackingPhase.CALIBRATE
        self.board_state: Dict[int, Tuple[str, str]] = {}
        
        self.detection_accum: Dict[int, List[Tuple[int, float]]] = defaultdict(list)
        self.accum_frames = 0
        self.detect_window = 5
    ...
    def calibrate(self, corners: np.ndarray):
        ...
        self.phase = TrackingPhase.DETECT
        print("Board calibrated - detecting initial position...")
    ...
    def _detect(self, frame: np.ndarray) -> List[Dict]:
        ...
        return self._cluster_detections(detections)
    ...
    def process(self, frame: np.ndarray) -> Optional[str]:
        ...
        for det in detections:
            ...
            if sq is not None:
                self.detection_accum[sq].append((det['class'], det['confidence']))
                
        self.accum_frames += 1
        
        if self.phase == TrackingPhase.DETECT:
            return self._process_detect()
        elif self.phase == TrackingPhase.TRACKING:
            return self._process_tracking()
            
        return None
        
    def _process_detect(self) -> Optional[str]:
        if self.accum_frames < self.detect_window:
            return None
            
        current = self._consolidate_detections()
        self.detection_accum.clear()
        self.accum_frames = 0
        
        if len(current) >= 30:
            print(f"  Detected {len(current)} pieces - locking initial position")
    ...
    def _consolidate_detections(self) -> Dict[int, Tuple[int, float]]:
        result = {}
        for sq, votes in self.detection_accum.items():
            if not votes:
                continue
            ...
            if vote_ratio >= 0.35 and avg_conf >= 0.25:
                result[sq] = (best_cls, avg_conf)
        return result
        
    def _process_tracking(self) -> Optional[str]:
        if self.accum_frames < self.detect_window:
            return None
        ...
        detected_squares = set(current.keys())
        ...
        move = self._find_best_move(vacated, appeared, current)
    ...
    def _find_best_move(self, vacated: Set[int], appeared: Set[int],
                        current: Dict[int, Tuple[int, float]]) -> Optional[chess.Move]:
        best_score = 0
        best_move = None
        
        for move in self.board.legal_moves:
            score = self._score_move(move, vacated, appeared, current)
            if score > best_score:
                best_score = score
                best_move = move
    ...
    def _score_move(self, move: chess.Move, vacated: Set[int], appeared: Set[int],
                    current: Dict[int, Tuple[int, float]]) -> int:
        score = 0
        ...
        if from_sq in self.board_state:
            ...
            if to_sq in current:
                detected_cls, det_conf = current[to_sq]
    ...
    def visualize(self, frame: np.ndarray) -> np.ndarray:
        ...
        cv2.putText(vis, phase_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        
        if self.phase == TrackingPhase.TRACKING and self.inv_transform is not None:
        ...
        if self.moves:
            move_str = f"Moves: {len(self.moves)} - " + " ".join(self.moves[-8:])
    ...
    def pgn(self, name: str) -> str:
        game = chess.pgn.Game()
        game.headers["Event"] = "ChessWorldAI V5"
...
def main():
    parser = argparse.ArgumentParser(description="ChessWorldAI V5 Tracker")
    ...
    output = args.output or f"output/pgn/{video.stem}_v5.pgn"
    ...
    tracker = SmartTrackerV5(model_path=model_path)
    ...
    try:
        while True:
            ...
            cv2.imshow("ChessWorldAI V5", vis)
